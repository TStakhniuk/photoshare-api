...
class PhotoTransformation(Base):
    ...
    url: Mapped[str] = mapped_column(String(500), nullable=False)
    cloudinary_public_id: Mapped[str] = mapped_column(String(255), nullable=False, unique=True)
    transformation_params: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON string
    qr_code_url: Mapped[str | None] = mapped_column(Text, nullable=True)  # Base64 QR code
    ...
    # Relationships
    original_photo: Mapped["Photo"] = relationship("Photo", back_populates="transformations")
